        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase">Khải Huyền Chưa Mở</h3>
              
              <div className="space-y-4 mb-10">
                <p className="text-amber-500 font-bold text-sm tracking-[0.3em] uppercase">Chủ đề: {selectedTopic}</p>
                <p className="text-slate-400 leading-relaxed text-lg font-light italic px-4">
                  "Sợi dây thiên mệnh về <span className="text-white font-medium">{selectedTopic}</span> đang được dệt lại bởi các Norn. Khi các tinh tú hội tụ đủ năng lượng, cánh cổng này sẽ tự khắc khai mở."
                </p>
              </div>
        ...
        <section className="max-w-7xl mx-auto px-6 pt-32 pb-32 md:pt-48 md:pb-56 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-12">
            ...
            <h1 className="text-6xl md:text-[90px] font-extrabold text-white leading-[1.1] tracking-tighter">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-white to-purple-400">Bản Ngã & Những Điều Sâu Thẳm</span>
            </h1>
            <p className="text-lg md:text-2xl text-slate-400 max-w-2xl mx-auto font-light">
               Vượt qua những bộn bề, hãy để chúng tôi cùng bạn đi sâu vào nội tâm và tìm thấy ánh sáng dẫn lối cho tương lai.
            </p>
        ...
        <section className="max-w-7xl mx-auto px-6 py-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 text-center md:text-left">
              <div className="space-y-4">
                  ...
                  <p className="text-slate-500 text-lg">Lựa chọn lĩnh vực bạn đang trăn trở.</p>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {topics.map((topic, index) => (
              ...
              >
                <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${topic.color} flex items-center justify-center mb-10 border border-white/10 group-hover:scale-110 transition-all ${topic.accent}`}>
                  <topic.icon className="w-8 h-8" />
                </div>
                <h3 className="text-2xl font-bold text-white mb-4">{topic.title}</h3>
                <p className="text-slate-500 text-base leading-relaxed">{topic.description}</p>
