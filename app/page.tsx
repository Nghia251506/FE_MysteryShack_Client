...
export default function Home() {
  ...
  const handleLogout = () => {
    dispatch(logout());
    router.refresh(); // Refresh lại trang để cập nhật UI
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 font-sans selection:bg-amber-500/30 overflow-x-hidden">
      ...
      {/* 1. HIỆU ỨNG NỀN HUYỀN BÍ (LAYERED BACKGROUND) */}
      ...
      {/* 2. NAVIGATION BAR (UI TỐI ƯU & LOGIC AUTH) */}
        ...
        <nav className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="relative">
                <div className="absolute inset-0 bg-amber-500/40 blur-md rounded-full scale-0 group-hover:scale-125 transition-transform duration-500" />
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            {/* --- LOGIC HIỂN THỊ THEO TRẠNG THÁI LOGIN --- */}
            {user ? (
      ...
      <main className="relative z-10">
        ...
        {/* 3. HERO SECTION (Hành trình khám phá bản ngã) */}
        <section className="max-w-7xl mx-auto px-6 pt-32 pb-32 md:pt-48 md:pb-56 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-white/5 border border-white/10 text-amber-400 text-xs font-black uppercase tracking-[0.3em] shadow-inner">
                <Sparkles className="w-4 h-4 animate-pulse" /> Trí tuệ cổ xưa trong kỷ nguyên số
            </div>

            <h1 className="text-6xl md:text-[92px] font-bold text-white leading-[1] tracking-tight">
              Hành Trình <span className="italic font-light">Khám Phá</span> <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-white to-purple-400">
                Bản Ngã Và Những Điều Sâu Thẳm
              </span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center pt-8">
              {/* Sửa Link: Dẫn vào trang Rút Bài thay vì Login */}
        ...
        {/* 4. CONSULTATION TOPICS (KHÍA CẠNH THAM VẤN) */}
        <section className="max-w-7xl mx-auto px-6 py-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
              <div className="space-y-4">
                  <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Cánh Cửa Tâm Hồn</h2>
                  <p className="text-slate-500 text-lg max-w-md">Lựa chọn lĩnh vực bạn đang trăn trở để nhận được lời hồi đáp chính xác nhất từ vũ trụ.</p>
              </div>
        ...
        {/* 5. HOW IT WORKS (HÀNH TRÌNH TRẢI NGHIỆM) */}
        ...
        {/* 6. CALL TO ACTION (KÊU GỌI ĐĂNG KÝ) */}
      ...
      {/* 7. MODERN FOOTER (ĐẲNG CẤP & TỐI GIẢN) */}
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-16">
            ...
            <div className="md:col-span-3 space-y-6">
                <h4 className="text-white font-bold text-sm uppercase tracking-widest">Nền tảng</h4>
                <ul className="space-y-4 text-sm text-slate-500">
                    <li><Link href="/about" className="hover:text-amber-400 transition-colors">Về chúng tôi</Link></li>
                    <li><Link href="/login" className="hover:text-amber-400 transition-colors">Dịch vụ Tarot</Link></li>
                    <li><Link href="#" className="hover:text-amber-400 transition-colors">Đội ngũ Reader</Link></li>
                    <li><Link href="#" className="hover:text-amber-400 transition-colors">Bảng giá</Link></li>
                </ul>
            </div>
            <div className="md:col-span-3 space-y-6">
                <h4 className="text-white font-bold text-sm uppercase tracking-widest">Pháp lý</h4>
                <ul className="space-y-4 text-sm text-slate-500">
                    <li><Link href="#" className="hover:text-amber-400 transition-colors">Điều khoản dịch vụ</Link></li>
                    <li><Link href="#" className="hover:text-amber-400 transition-colors">Chính sách bảo mật</Link></li>
                    <li><Link href="#" className="hover:text-amber-400 transition-colors">Quyền riêng tư</Link></li>
                </ul>
            </div>
          </div>
          ...
          <div className="mt-24 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
              ...
              <div className="flex gap-12">
                  <div className="w-5 h-5 bg-slate-900 rounded-full hover:bg-amber-500/50 transition-colors cursor-pointer border border-white/5" />
                  <div className="w-5 h-5 bg-slate-900 rounded-full hover:bg-amber-500/50 transition-colors cursor-pointer border border-white/5" />
                  <div className="w-5 h-5 bg-slate-900 rounded-full hover:bg-amber-500/50 transition-colors cursor-pointer border border-white/5" />
              </div>
          </div>
