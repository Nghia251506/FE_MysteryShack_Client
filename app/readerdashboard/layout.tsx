      ...
      <aside className="hidden lg:flex w-64 bg-[#130823]/60 backdrop-blur-xl border-r border-white/5 flex-col h-screen sticky top-0 z-50">
        ...
        <div className="p-4 m-4 space-y-3">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${isActive ? "border-green-500/50 text-green-400" : "border-white/5 text-slate-500"}`}
          >
            <Power className="w-4 h-4" />{" "}
            <span className="text-[10px] font-bold uppercase">
              {isActive ? "Sẵn sàng" : "Đang nghỉ"}
            </span>
          </button>
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 py-2 text-red-400/60 border border-red-500/10 rounded-xl text-[10px] font-bold uppercase hover:bg-red-500/20"
          >
