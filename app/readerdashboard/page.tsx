...
export default function ReaderDashboardPage() {
  ...
  // 1. Lấy dữ liệu từ Redux (Ép kiểu any để tránh lỗi TypeScript gán nhầm HistoryState)
  const { user } = useAppSelector((state: any) => state.auth);
  const ratingState = useAppSelector((state: any) => state.rating);
  const rStats = ratingState.stats;
  const readerId = user?.id;

  // 2. State local quản lý thu nhập (Đổi tên để tránh trùng với rStats)
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Gọi song song các API và Dispatch
      const [income, sessions, activeSession] = await Promise.all([
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8 pb-32">
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-2">
        <div className="flex-1">
          <h1 className="text-3xl font-black text-white uppercase tracking-tighter italic">
            Dashboard <span className="text-amber-500">Reader</span>
          </h1>
          ...
          {processingSession ? (
            ...
            >
              ...
              <div className="relative bg-[#1a0b2e]/90 border border-red-500/20 backdrop-blur-2xl p-3 px-6 rounded-[2rem] flex items-center gap-4">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${timeLeft < 600 ? "bg-red-600 animate-pulse" : "bg-amber-500"}`}
                >
                ...
                <div className="flex flex-col min-w-[160px]">
                  ...
                  <p className="text-xs font-bold text-slate-200 mt-0.5 italic">
                    Phiên luận giải hiện tại
                  </p>
                  <div className="flex items-center gap-1.5 mt-1 text-sm font-mono font-black">
                    <span
                      className={
                        timeLeft < 600 ? "text-red-500" : "text-amber-400"
                      }
                    >
                      {formatTime(timeLeft)}
                    </span>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 p-3 px-6 rounded-[2rem] flex items-center gap-3 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
              {/* Icon Green với hiệu ứng nháy nhẹ (pulse) để báo hiệu đang active */}
          ...
          <div className="bg-white/5 border border-white/10 px-4 py-3 rounded-2xl flex items-center gap-2 hover:scale-105 transition-transform">
            ...
            <span className="text-white font-bold text-sm">
              {rStats?.averageRatingMonth
                ? rStats.averageRatingMonth.toFixed(1)
                : "0.0"}
            </span>
          </div>
          <div className="bg-[#130823]/80 border border-amber-500/20 px-4 py-3 rounded-2xl flex items-center gap-2">
            <Trophy className="w-4 h-4 text-amber-500" />
            <span className="text-[10px] text-slate-300 font-black uppercase leading-none">
              Top Reader
              <br />
              Yêu thích
            </span>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div
          whileHover={{ y: -5 }}
          className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden"
        >
          ...
          <h2 className="text-4xl font-black mt-2 italic">
            +{incomeStats.todayIncome.toLocaleString()}đ
          </h2>
        ...
        <motion.div
          whileHover={{ y: -5 }}
          className="bg-[#130823]/80 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 flex flex-col justify-between hover:border-amber-500/30 transition-all shadow-xl shadow-black/50"
        >
          ...
          <div className="mt-8">
            <p className="text-3xl font-black text-white italic">
              {rStats?.totalReviewsMonth || 0} Lượt
            </p>
        ...
        >
          ...
          <div className="mt-8">
            <p className="text-3xl font-black text-white italic">
              {incomeStats.totalSessions} Phiên
            </p>
            <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
        ...
        <div className="flex flex-col items-center md:items-end gap-4 z-10 w-full md:w-80">
          <div className="w-full">
            <div className="flex justify-between text-[10px] font-black uppercase mb-3 text-white tracking-widest">
              <span className="text-amber-500">Lượt nhận khách</span>
              <span>
          ...
          <button className="w-full md:w-auto mt-2 px-8 py-3 bg-white text-black text-xs font-black uppercase rounded-2xl hover:bg-amber-500 hover:text-white transition-all transform active:scale-95 flex items-center justify-center gap-3">
            Gia hạn gói <Zap className="w-4 h-4 fill-current" />
          </button>
        </div>
      </section>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium italic">
                Đối soát chi tiết các phiên đã xong
              </p>
            </div>
          </div>
        ...
        <div className="bg-[#130823]/40 border border-white/5 rounded-[2rem] p-6 flex items-center justify-between group cursor-pointer hover:bg-white/5 hover:border-amber-500/40 transition-all shadow-md">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium italic">
                Báo cáo tỷ lệ hài lòng từ khách
              </p>
            </div>
          </div>
          <ArrowUpRight className="w-5 h-5 text-slate-600 group-hover:text-amber-500 transition-colors" />
