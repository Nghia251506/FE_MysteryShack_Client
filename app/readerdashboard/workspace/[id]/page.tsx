...
export default function WorkspacePage() {
  ...
  const transformData = (item: any) => {
    ...
    return {
      ...
      question: item.question?.content || item.question || "Không có câu hỏi cụ thể",
    ...
    const loadDetail = async () => {
      try {
        ...
        if (response) {
          ...
          // Nếu đã hoàn thành hoặc đang chờ thanh toán thì chuyển trạng thái UI luôn
  ...
  const handleSubmit = async () => {
    ...
    // Kiểm tra xem đã nhập đủ chưa (Logic cũ của ông)
    const filledCards = Object.keys(cardInputs).length;
    if (filledCards < activeRequest.cards.length || !summary) {
    ...
    try {
      ...
      toast.error("Gửi kết quả thất bại. Vui lòng thử lại.");
    } finally {
  ...
  if (!activeRequest) return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#050505] space-y-4">
      ...
      <div className="text-slate-400 font-mono tracking-widest text-xs uppercase animate-pulse">
        Đang mở kết nối với các lá bài...
      </div>
    </div>
  );
      ...
      <AnimatePresence mode="wait">
        {!isSent ? (
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
            ...
            {/* Header Workspace (Giao diện ông gửi) */}
            <div className="bg-[#130823]/60 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-2xl">
              ...
              <div className="flex flex-col items-center px-8 py-5 bg-gradient-to-b from-white/5 to-transparent rounded-[2rem] border border-white/10 min-w-[160px] shadow-inner">
              ...
              <div className="pt-10 border-t border-white/10 grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-br from-white/10 to-transparent rounded-[2rem] p-8 border border-white/10 flex flex-col sm:flex-row gap-8 items-center shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-green-500/10 blur-[50px] rounded-full pointer-events-none group-hover:bg-green-500/20 transition-all"></div>
                    <div className="p-3 bg-white rounded-2xl shadow-white/5 shadow-2xl relative z-10 rotate-3 group-hover:rotate-0 transition-transform duration-500">
                    ...
                    <div className="space-y-4 flex-1 relative z-10 text-center sm:text-left">
                      ...
                      <p className="text-[11px] text-slate-400 font-medium leading-relaxed italic">Mã QR này được tạo tự động cho riêng phiên luận giải của {activeRequest.querentName}.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="fixed bottom-8 left-0 lg:left-64 right-0 z-40 px-6 flex justify-center">
              <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="bg-[#1a1025]/90 border border-white/10 p-3 pl-8 rounded-[2rem] flex items-center gap-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl ring-1 ring-white/5">
                ...
                <button 
                  ...
                  className="bg-gradient-to-r from-amber-600 to-orange-600 px-10 py-4 rounded-2xl text-white font-black uppercase text-xs tracking-[0.2em] flex items-center gap-3 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 shadow-xl shadow-orange-900/20"
                >
          ...
          <motion.div key="success" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center min-h-[75vh]">
            <div className="bg-[#130823]/80 backdrop-blur-2xl border border-green-500/20 rounded-[4rem] p-16 text-center shadow-[0_0_100px_rgba(34,197,94,0.1)] relative overflow-hidden">
              ...
              <h2 className="text-5xl font-black text-white mb-4 tracking-tighter uppercase">Đã Gửi Thành Công!</h2>
              <p className="text-slate-400 text-lg mb-12 max-w-md mx-auto font-medium">Thông điệp tâm linh đã được gửi tới khách hàng. Vui lòng kiểm tra tài khoản và xác nhận sau khi nhận được tiền.</p>
              
              <div className="flex flex-col gap-4">
                <button 
                  onClick={handleConfirmPayment} 
                  disabled={confirmingPayment} 
                  className="px-12 py-5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-black rounded-[2rem] transition-all flex items-center gap-3 mx-auto shadow-2xl shadow-green-900/20 active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm"
                >
