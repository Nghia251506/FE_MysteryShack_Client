...
export default function BookingRequestPage() {
  ...
  const handleCreateBooking = async () => {
    ...
    const backupString = session.drawnCards
        .map((c:any) => `${c.id || c.dbId || 0}-${c.isReversed ? 1 : 0}`)
        .join(',');

    const cardsPayload = session.drawnCards.map((c: any, index: number) => {
        const id = Number(c.id || c.dbId || 0);
        const fallback = getCardFallback(id);
        const realName = c.name || c.nameVi || fallback.name;
        const realImg = c.img || c.image || c.imageUrl || fallback.img;
    ...
    const payload: any = {
        ...
        active: true,
        amount: 50000, 
        note: `KH: ${formData.name} - Hỏi: ${formData.question} || BACKUP_CARDS:${backupString}`,
    ...
    try {
        ...
        // Thay alert bằng modal thành công
