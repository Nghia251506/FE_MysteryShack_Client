  ...
  const renderCard = (card: any, globalIdx: number) => (
    ...
    >
      ...
      >
        ...
        <div
          className="absolute inset-0 w-full h-full rounded-md md:rounded-xl shadow-lg bg-[#12081d]"
    ...
    <motion.div className="flex flex-col items-center w-full min-h-[90vh] pt-6 md:pt-10 px-2 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-1 md:gap-2 w-full max-w-5xl items-center opacity-80">
        {Array.from({ length: rowCount }).map((_, rowIndex) => (
          <div
            key={rowIndex}
            className="flex justify-center items-center w-full pl-4"
          >
            {shuffledDeck
      ...
      <p className="mt-8 text-amber-200 text-sm font-semibold uppercase tracking-[0.15em] text-center drop-shadow-[0_0_8px_rgba(251,191,36,0.4)]">
        {selectedIndices.length < 3
          ? `Cần chọn thêm ${3 - selectedIndices.length} lá để giải mã`
          : "Dòng chảy năng lượng đã sẵn sàng"}
      </p>
    </motion.div>
  );
};
